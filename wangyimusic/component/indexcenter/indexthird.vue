...
<script setup>
	import { xunhuan , dt } from '../../api/lunbotu/lb.js'
	...
	// let listenmusic = (i) => {
	// 	uni.navigateTo({
	// 		url:`/pages/bofang/bofang?id=${i.id}&img=${i.picUrl}`
	// 	})
	// 	// console.log(i);
	// }
		...
		dt()
			.then(
				res => {
					// console.log(res);
					let state=-1;
					 let newArr= res.djRadios.reduce((total,item,index)=>{
						if(index%3==0){
						 state++;
						 let newArr=[]
						 newArr.push(item)
